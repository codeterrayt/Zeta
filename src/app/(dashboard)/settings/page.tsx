...
import { Save, Palette, TrendingUp, ShieldCheck, Loader2, History, Bell, Settings } from "lucide-react"
...
export default function SettingsPage() {
  ...
  const isAdmin = session?.user?.role === "ADMIN" || session?.user?.isOwner
  ...
  return (
    <div className="p-6 lg:p-10 space-y-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2 font-medium text-lg">
            Manage your workspace preferences, performance thresholds, and administrative settings.
          </p>
        </div>
      </header>

      {/* Tabs for Admins / Owners */}
      {isAdmin && (
        <div className="flex border-b border-border/60 gap-6">
          <button
            onClick={() => setActiveTab("preferences")}
            className={cn(
              "flex items-center gap-2 px-4 py-3 font-black text-sm border-b-2 transition-all cursor-pointer",
      ...
      {activeTab === "admin" && isAdmin ? (
        <AdminSettings currentUserId={currentUserId} isOwner={isOwner} />
      ) : (
        <div className="grid grid-cols-1 gap-8">
          ...
          <section className="bg-card border border-border/60 rounded-[2.5rem] shadow-sm overflow-hidden transition-all hover:shadow-xl">
            ...
            <div className="p-8 border-t border-border/40">
              <p className="text-xs text-muted-foreground font-medium">
                When enabled, Zeta will ask you to add optional comments for changes when saving a task, helping your team understand the context behind each timeline event.
              </p>
            </div>
          </section>
          ...
          <section className="bg-card border border-border/60 rounded-[2.5rem] shadow-sm overflow-hidden transition-all hover:shadow-xl">
            ...
            <div className="p-8 border-t border-border/40">
              <p className="text-xs text-muted-foreground font-medium">
                When enabled, Zeta will trigger instant floating popup alerts for active ticket changes, sprint activity comments, mentions, and key updates while you are in the application.
              </p>
            </div>
          </section>
